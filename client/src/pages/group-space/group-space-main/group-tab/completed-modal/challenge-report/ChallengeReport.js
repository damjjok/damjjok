...
import Strick from "../../home-tab/strick/Strick";
...
function ChallengeReport({nextContent}) {
  ...
  <>
        <Flex flexFlow={'column'} justifyContent={'center'} alignItems={'center'} marginY={20}>
            <ModalBody>
              <VStack flexFlow={'column'} justifyContent={'center'} alignItems={'center'}>
                {/* <StarIcon marginY={4} boxSize={20} color='dam.yellow'/> */}
                <img src={trophyIcon} alt="trophyIcon" style={{
                    clipPath: 'circle(50%)', 
                    width: '100px', 
                    height: '100px',
                    objectFit: 'cover',
                }}/>
                <p className=" font-extrabold text-4xl text-center">축하해요!</p>
                <p className=" font-extrabold text-4xl text-center">앞으로도 금연을 이어나가요!</p>
                <Flex flexFlow={'column'} marginY={10}>
                  <Text fontSize={'xl'} fontWeight={'semibold'}>180일 챌린지에 성공했어요!</Text>
            ...
            <BasicButton buttonName={'다음으로'} variant={'bigbtn'} onClick={nextContent}/>
        </Flex>
  </>)
}
